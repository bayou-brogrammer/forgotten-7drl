...
impl GameOfLife {
    ...
    fn step(
        &mut self,
        &GameOfLifeParams { survive_min, survive_max, resurrect_min, resurrect_max }: &GameOfLifeParams,
    ) {
        for ((coord, &alive_cell), next_cell) in self.alive.enumerate().zip(self.next.iter_mut()) {
            let n: u8 = Direction::all()
                .map(|direction| self.alive.get(coord + direction.coord()).cloned().unwrap_or(false) as u8)
                .sum();
...
fn generate_initial_cavern_map(size: Size) -> Grid<FloorOrWall> {
    ...
    let params = GameOfLifeParams { survive_min: 4, survive_max: 8, resurrect_min: 5, resurrect_max: 5 };

    for _ in 0..NUM_STEPS {
        game_of_life.step(&params);
    }

    Grid::new_grid_map(game_of_life.alive, |alive| if alive { FloorOrWall::Floor } else { FloorOrWall::Wall })
}
...
fn remove_disconnected_walls(map: &mut Grid<FloorOrWall>) {
    assert!(*map.get_checked(Coord::new(0, 0)) == FloorOrWall::Wall, "top-left cell must be wall");
