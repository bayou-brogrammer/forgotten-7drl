...
fn is_cell_in_corridor_axis(map: &Grid<FloorOrWall>, coord: Coord, axis: Axis) -> bool {
    ...
    let floor_in_axis =
        *map.get_checked(coord + axis_delta) == Floor && *map.get_checked(coord - axis_delta) == Floor;
...
impl RoomPlacement {
    ...
    fn try_add_room(&mut self, new_room: Room) {
        ...
        for &existing_room in crate::rng::choose_multiple(&self.rooms, 2) {
            ...
            let corridor = l_shaped_corridor(new_room.rect.centre(), existing_room.rect.centre(), &self.map);
