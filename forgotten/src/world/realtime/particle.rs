...
use gridbugs::{
    ...
    visible_area_detection::{vision_distance::Circle, Light},
};
...
pub mod spec {
    use crate::particle::Light;
    pub use crate::world::Tile;
    pub use gridbugs::rgb_int::Rgb24;
    use gridbugs::visible_area_detection::vision_distance::Circle;
    ...
    pub struct Particle {
        ...
        pub possible_light: Option<Possible<Light<Circle>>>,
...
impl spec::Movement {
    fn choose<R: Rng>(&self, rng: &mut R) -> movement::MovementState {
        ...
        let radial = Radial { angle, length: VECTOR_LENGTH };
...
impl spec::ParticleEmitter {
    pub fn build(self) -> ParticleEmitterState {
        ...
        ParticleEmitterState {
            ...
            fade_out_state: self
                .fade_out_duration
                .map(|d| FadeOutState { total: d, elapsed: Duration::from_millis(0) }),
...
impl RealtimeComponent for ParticleEmitterState {
    ...
    fn tick(&mut self) -> (Self::Event, Duration) {
        ...
        let (fade_state, light_colour_fade_state) = match self.fade_out_state.as_mut() {
            None => (
                ...
                self.particle_spec.light_colour_fade.as_ref().map(|l| {
                    ...
                    LightColourFadeState { fade_state, from: l.from, to: l.to }
                }),
            ),
            Some(fade_out_state) => {
        ...
        let event = SpawnParticle {
            movement_state: self.particle_spec.movement.as_ref().map(|m| m.choose(&mut self.rng)),
            fade_state,
            tile: self.particle_spec.tile,
            colour_hint: self.particle_spec.colour_hint.map(|c| c.choose(&mut self.rng)),
            light_colour_fade_state,
            light: self.particle_spec.possible_light.as_ref().and_then(|l| l.choose(&mut self.rng)),
            particle_emitter: self
                .particle_spec
                .possible_particle_emitter
                .as_ref()
                .and_then(|p| p.choose(&mut self.rng).map(|p| Box::new(p.build()))),
            damage: self.particle_spec.possible_damage.as_ref().and_then(|d| {
                d.choose(&mut self.rng).map(|d| ProjectileDamage {
...
impl<'a> RealtimeComponentApplyEvent<Context<'a>> for ParticleEmitterState {
    fn apply_event(mut spawn_particle: SpawnParticle, entity: Entity, context: &mut Context<'a>) {
        ...
        if let Some(movement) = spawn_particle.movement_state.take() {
            context.world.realtime_components.movement.insert_with_schedule(
                particle_entity,
        ...
        context.world.spatial_table.update_coord(particle_entity, coord).unwrap();
        context.world.components.particle.insert(particle_entity, ());
        ...
        if let Some(fade_state) = spawn_particle.fade_state {
            context.world.realtime_components.fade.insert(particle_entity, fade_state);
        }
        context.world.components.realtime.insert(particle_entity, ());
        if let Some(colour_hint) = spawn_particle.colour_hint {
            context.world.components.colour_hint.insert(particle_entity, colour_hint);
        }
        if let Some(light) = spawn_particle.light.take() {
            context.world.components.light.insert(particle_entity, light);
        }
        if let Some(light_colour_fade) = spawn_particle.light_colour_fade_state.take() {
            context.world.realtime_components.light_colour_fade.insert(particle_entity, light_colour_fade);
        }
        if let Some(particle_emitter) = spawn_particle.particle_emitter.take() {
            context.world.realtime_components.particle_emitter.insert(particle_entity, *particle_emitter);
        }
        if let Some(projectile_damage) = spawn_particle.damage.take() {
            context.world.components.projectile_damage.insert(particle_entity, projectile_damage);
        }
    }
}
