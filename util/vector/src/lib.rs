...
impl Cartesian {
    pub fn from_coord(coord: Coord) -> Self {
        Self { x: coord.x as f64, y: coord.y as f64 }
    }
...
impl Radial {
    pub fn to_cartesian(self) -> Cartesian {
        Cartesian { x: self.length * self.angle.0.cos(), y: self.length * self.angle.0.sin() }
    }
    pub fn rotate_clockwise(self, angle: Radians) -> Self {
        Self { angle: Radians(self.angle.0 + angle.0), length: self.length }
    }
}
...
impl UniformSampler for UniformRadians {
    ...
    {
        Self { inner: UniformFloat::<f64>::new(low.borrow().0, high.borrow().0) }
    }
